...
/// <summary>
/// Keeps the Diagnostics table of <c>src/SsalKit.Guard/AI.md</c> in agreement with the live
/// descriptor table.
/// </summary>
/// <remarks>
...
public class AiDocDiagnosticTableTests
{
    ...
    /// backtick-quoted id. The "diagnostic consequences" table earlier in the document lists several
    /// ids in one cell and deliberately does not match, because the closing backtick must be
    /// followed by the cell separator.
    /// </summary>
    ...
    private static string AiDocPath([CallerFilePath] string testFilePath = "") =>
        Path.GetFullPath(
            Path.Combine(Path.GetDirectoryName(testFilePath)!, "..", "..", "src", "SsalKit.Guard", "AI.md"));
}
