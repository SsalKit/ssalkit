...
1 つの例外が得るのはファクトリー 1 つと throw ヘルパー 1 つだけです — コンストラクターごとにオーバーロードが生えるのではなく、宣言されているうち最も広い形状 1 つだけがミラーリングされます。それより狭い形は、その 1 つの既定値で到達できるからです。
...
| `SSALG002` | エラー | `[ErrorCodes]` コンテナーが、生成ファイルが 2 つ目のパートを付けられる `static partial class` ではない — クラスでない、`static` でない、`partial` でない、`file`-local、または `partial` でない型の中にネストされている。 |
...
| `SSALG007` | エラー | `[ErrorCodes]` コンテナーがジェネリック、またはジェネリック型の中にネストされている。あるいはコード enum がジェネリック型の中にネストされている。 |
...
| `SSALG010` | 警告 | `[ExternalErrorCode<TCode>]` がコンテナーの `[ErrorCodes<TCode>]` と別のコード enum を指定しており、どのコンテナーにも属さず捨てられる。 |
| `SSALG011` | 警告 | コンテナーのコード enum が別のアセンブリで宣言されていて、このコンパイル内で登録されるものが 1 つもなく、生成されるマッピングが空になる。 |

1 つの登録に関するルール（`SSALG001`、`SSALG004`、`SSALG005`、`SSALG009`、`SSALG010`）は、その登録だけを捨ててコンテナーの残りはそのまま残します — 宣言を誤った例外 1 つがマッピングテーブル全体を道連れにすべきではないからです。コンテナー自体に関するルール（`SSALG002`、`SSALG007`）や、ジェネレーターが利用者に代わって解決することを拒む曖昧さ（`SSALG003`）は、そのコンテナーの生成ファイルを丸ごと抑制します。
...
- **1 クラスに 1 コンテナー — 言語がそう決めています。** 1 つのクラスに `[ErrorCodes<A>]` と `[ErrorCodes<B>]` を同時に付けると「属性が重複しています」（`CS0579`）のエラーになります。ジェネリック属性の `AllowMultiple = false` は、構築された個々の形ではなく属性の*定義*に対して適用されるからです。1 つの例外に `[ErrorCode<A>]` と `[ErrorCode<B>]` を付ける場合も同じです。コード enum をもう 1 つ使いたければコンテナークラスをもう 1 つ作ればよく、そもそもここでの設計はすべてその形を前提にしています。
- **ジェネレーターが見えるのは 1 つのコンパイルだけです。** コンテナーが集めるのは、自分がコンパイルされるアセンブリの `[ErrorCode]` 例外だけです。2 つのプロジェクトがコード enum を共有していても、参照先プロジェクトにある例外はコンテナーからは見えません。コンテナーはその例外たちと同じプロジェクトに置いてください。よそにある型（参照先プロジェクトの例外やサードパーティーの型）をマップしたいときは `[ExternalErrorCode]` で明示的に登録すればよく、こちらはアセンブリ境界を越えます。別アセンブリのコード enum を担当するコンテナーが結局空になった場合は、黙って空のテーブルができる代わりに警告（`SSALG011`）が出ます。
