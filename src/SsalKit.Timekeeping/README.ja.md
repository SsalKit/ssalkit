...
- **次の単位に向けた部分的な進捗が正確に保存されます。** プールがすでに満タンの状態で 1 単位消費すると、`FullAt` は `RechargeEvery` 1 つ分だけ前に押し出されるだけで、すでに進んでいたチャージの進捗はリセットされません。同じ時刻に同じ量を `Grant` すると、その移動をちょうど打ち消します。
