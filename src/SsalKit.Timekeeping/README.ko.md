...
- **다음 충전을 향한 부분 진행률이 정확히 보존됩니다.** 풀이 이미 가득 찬 상태에서 한 개를 소비하면 `FullAt`이 `RechargeEvery` 하나만큼 앞으로 밀릴 뿐, 이미 진행 중이던 충전의 진행률을 초기화하지 않습니다. 같은 시각에 같은 양을 `Grant`하면 정확히 그 이동을 되돌립니다.
...
확장 메서드는 `GetUtcNow()`만 호출하므로, 그 밖에 가짜로 만들 것이 없습니다. `Cooldown`과 `RechargePool`도 같은 방식으로 테스트합니다 — 시각을 직접 넘기거나, 같은 가짜 `TimeProvider`를 확장 메서드에 넘기세요.
