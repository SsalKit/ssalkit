...
/// <summary>
/// Renders the collected <see cref="ClassRegistrationModel"/>s, <see cref="ConventionRegistrationModel"/>s,
/// and <see cref="ServiceFactoryModel"/>s for an assembly into a single, deterministic C# source
/// file containing one <c>IServiceCollection</c> extension method.
/// </summary>
internal static class ServiceRegistrationEmitter
{
    ...
    /// <summary>
    /// Renders the assembly's registration extension method.
    /// </summary>
    /// <param name="classes">Every class carrying at least one valid <c>[Service]</c> attribute.</param>
    /// <param name="conventions">
    /// Every registration produced by an <c>[assembly: RegisterImplementationsOf]</c> convention
    /// scan, already ordered and deduplicated by <see cref="Parsing.ConventionScanner"/>. Emitted as
    /// its own block after the <c>[Service]</c> registrations, for the same reason the factory block
    /// is kept separate: an assembly that declares no contracts produces byte-for-byte the same file
    /// it did before this feature existed.
    /// </param>
    /// <param name="factories">
    /// Every valid <c>[ServiceFactory]</c> interface. Their registrations are appended after the
    /// <c>[Service]</c> ones as a separate, independently-sorted block rather than interleaved: the
    /// generated implementation types all live under one namespace, so sorting them together with
    /// the <c>[Service]</c> implementations would wedge them into an arbitrary position in that
    /// list. Keeping the block separate also means an assembly with no factories at all produces
    /// byte-for-byte the same file it did before this feature existed.
    /// </param>
    /// <param name="assemblyName">The compilation's assembly name, which the class and method are named after.</param>
    public static (string HintName, string Source) Emit(
        ImmutableArray<ClassRegistrationModel> classes,
        ImmutableArray<ConventionRegistrationModel> conventions,
        ImmutableArray<ServiceFactoryModel> factories,
        string? assemblyName)
    {
        ...
        var orderedClasses = classes.IsDefaultOrEmpty
            ? ImmutableArray<ClassRegistrationModel>.Empty
            : classes.Sort((a, b) => string.CompareOrdinal(a.ImplementationTypeFqn, b.ImplementationTypeFqn));

        var orderedFactories = factories.IsDefaultOrEmpty
            ? ImmutableArray<ServiceFactoryModel>.Empty
            : factories.Sort((a, b) => string.CompareOrdinal(a.InterfaceTypeFqn, b.InterfaceTypeFqn));

        // Already ordered (and deduplicated) by ConventionScanner, which is the only place that
        // knows how a scan's registrations should be grouped: by declared contract first.
        var orderedConventions = conventions.IsDefault ? ImmutableArray<ConventionRegistrationModel>.Empty : conventions;
        ...
        // Written only when there is something to describe, which is what keeps the emitted file
        // byte-for-byte identical to what it was before [RegisterImplementationsOf] existed.
        if (orderedConventions.Length > 0)
        {
            sb.AppendLine("    /// Also registers every class matched by an");
            sb.AppendLine("    /// <c>[assembly: RegisterImplementationsOf]</c> contract declared in this assembly.");
        }

        // Written only when there is something to describe, which is what keeps the emitted file
        // byte-for-byte identical to what it was before [ServiceFactory] existed.
        if (orderedFactories.Length > 0)
        {
            sb.AppendLine("    /// Also registers the generated implementation of every interface decorated with");
            sb.AppendLine("    /// <c>[ServiceFactory]</c> as a singleton.");
        }
        ...
        {
            // Reuses the [Service] statement writer wholesale: a convention registration is just a
            // single-service-type, never-keyed, never-factory entry, so forwarding can never apply
            // (RequiresForwarding needs 2+ service types) and every mode/lifetime/open-generic shape
            // is rendered exactly as the equivalent hand-written [Service] would be.
            var entry = new RegistrationEntryModel(
                ImmutableArray.Create(convention.ServiceTypeFqn).ToEquatableArray(),
                convention.Lifetime,
                convention.Mode,
                KeyModel.None,
                convention.IsOpenGeneric,
                FactoryModel.None);

            WriteRegistration(sb, "            ", convention.ImplementationTypeFqn, entry);
        }

        foreach (var factory in orderedFactories)
        {
            // Always Singleton and always non-keyed: the implementation is a stateless wrapper over
            // the IServiceProvider, and it is the *keyed* registrations it looks up that carry the
            // lifetimes that matter.
            sb.Append("            ").Append(ServicesParameter)
              .Append(".AddSingleton<").Append(factory.InterfaceTypeFqn)
              .Append(", ").Append(factory.ImplementationTypeFqn).AppendLine(">();");
        }
